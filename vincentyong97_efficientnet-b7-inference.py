...
os.listdir("../input/efficientnet-mixup-256")
...
MODEL = '../input/efficientnet-mixup-256/efficientnet_with_mixup_256.pth'
nworkers = 2
...
class Head(nn.Module):
    def __init__(self, nc, n, ps=0.5):
        super().__init__()
        layers = [AdaptiveConcatPool2d(), Mish(), Flatten()] +             bn_drop_lin(nc*2, 512, True, ps, nn.ReLU(inplace=True)) +             bn_drop_lin(512, n, True, ps)
...
class EfficientNet_1Channel(nn.Module):
    def __init__(self, arch=arch, n=nunique, pre=True, ps=0.5):
        ...
        _conv_stem = Conv2dStaticSamePadding(1, 48, kernel_size=(3, 3), stride=(2, 2), bias=False, image_size=[600,600])
        w = (model._conv_stem.weight.sum(1)).unsqueeze(1)
        _conv_stem.weight = nn.Parameter(w)

        self.layer0 = nn.Sequential(_conv_stem, model._bn0)
        self.layer1 = nn.Sequential(*model._blocks[0:])
        self.layer2 = nn.Sequential(model._conv_head)
        self.layer3 = nn.Sequential(nn.BatchNorm2d(2560, eps=0.001, momentum=0.010000000000000009, affine=True, track_running_stats=True),
                                    nn.AdaptiveAvgPool2d(output_size=1),
                                    nn.Dropout(p=0.5, inplace=False))

        # nc = self.layer3[-1].weight.shape[0]
        self.head1 = Head(2560,n[0])
        self.head2 = Head(2560,n[1])
        self.head3 = Head(2560,n[2])
        to_Mish(self.layer0), to_Mish(self.layer1), to_Mish(self.layer2)
        to_Mish(self.layer3)
...
model = EfficientNet_1Channel().cuda()
