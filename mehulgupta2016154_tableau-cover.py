...
train1=rs().fit_transform(train)
test1=rs().fit_transform(test)
xtrain,xtest,ztrain,ztest=tts(train,Target,train_size=0.6)
...
xg={'n_estimators':[200],'learning_rate':[0.08],'max_depth':[15],'objective':['multi:softmax'],'subsample':[0.6],'colsample_bytree':[0.6]}
model=grc(xgb(),xg,cv=3)


# In[ ]:


model.fit(xtrain1,ztrain1,verbose=1)
...
print(acs(ztest1,model.predict(xtest1)))
