...
new_num_arr = np.load('data_new.npy') #
...
final_num_arr=np.concatenate((train_num_arr,new_num_arr))
...
autoencoder.fit(final_num_arr,final_num_arr,epochs=13,batch_size=10)


# In[15]:


encoded_train=encoder.predict(train_num_arr)
encoded_test=encoder.predict(new_num_arr)


# In[16]:


encoder.predict(new_num_arr)[4]


# In[17]:


image1=autoencoder.predict(new_num_arr)[4]
plt.figure(figsize=(10,10))
plt.imshow(np.reshape(image1[10],(128,128)),cmap=plt.cm.bone)


# In[18]:


X1=final[['base_fvc','base_percent','Age','sex','smokingstatus','weeks_passed','base_week']].copy()
y=final.FVC.copy()
...
X_test=test[['base_fvc','base_percent','Age','sex','smokingstatus','weeks_passed','base_week']].copy()
...
model.fit(X1,y1,batch_size=128,epochs=200)
