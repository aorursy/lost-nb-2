...
df_train = pd.read_csv("../input/train.csv", nrows = 30_000, parse_dates=["pickup_datetime"])
df_test = pd.read_csv("../input/test.csv",nrows = 30_000)
print(">>  Data Loaded")
...
print(f"Numer of Missing values in train: ", df_train.isnull().sum().sum())
print(f"Number of Missing values in test: ", df_test.isnull().sum().sum())


# In[9]:


print(f">> Data Available since {df_train.key.min()}")
print(f">> Data Available upto {df_train.key.max()}")
...
data = [go.Scattermapbox(
            ...
            lon= df_train['pickup_longitude'],
            customdata = df_train['key'],
            mode='markers',
            marker=dict(
                size= 4,
                color = 'gold',
...
data = [go.Scattermapbox(
            lat= df_train['dropoff_latitude'] ,
            lon= df_train['dropoff_longitude'],
            customdata = df_train['key'],
            mode='markers',
            marker=dict(
                size= 4,
...
df_train['pickup_datetime_day_of_week_name'] = df_train['pickup_datetime'].dt.weekday_name
...
business_train = df_train[df_train['pickup_datetime_day_of_week'] < 5 ]
