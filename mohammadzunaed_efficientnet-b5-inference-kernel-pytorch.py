...
class CNN_Model(nn.Module):
    def __init__(self,model_name='efficientnet-b5',global_dim=2048):        
        ...
        self.cnn_backbone = EfficientNet.from_name(model_name)
...
checkpoint_dict = torch.load('/kaggle/input/modelsubmission46/checkpoint_best_Average_recall.pth')
model_1.load_state_dict(checkpoint_dict['Model_state_dict'])
model_1 = model_1.cuda()
model_1.eval()

print('Validation Average Recall {}          \nValidation Grapheme Root Recall {}         \nValidation Vowel Diacritic Recall {}         \nValidation Consonant Diacritic Recall {}         \nValidation Loss {} '.format(checkpoint_dict['Current_average_recall'],checkpoint_dict['Current_grapheme_root_recall'],checkpoint_dict['Current_vowel_diacritic_recall'],                                     checkpoint_dict['Current_consonant_diacritic_recall'],checkpoint_dict['Current_val_Loss']))
...
checkpoint_dict = torch.load('/kaggle/input/modelsubmission58/checkpoint_best_Average_recall.pth')
...
checkpoint_dict = torch.load('/kaggle/input/modelsubmission57/checkpoint_best_Average_recall.pth')
...
checkpoint_dict = torch.load('/kaggle/input/modelsubmission73/checkpoint_best_Average_recall.pth')
model_4.load_state_dict(checkpoint_dict['Model_state_dict'])
...
checkpoint_dict = torch.load('/kaggle/input/modelsubmission74/checkpoint_best_Average_recall.pth')
model_5.load_state_dict(checkpoint_dict['Model_state_dict'])
...
all_grapheme_labels = []
all_vowel_diacritic_labels = []
all_consonant_diacritic_labels = []
...
with torch.no_grad():
    for fname in TEST:
        ...
        ds_test = BengaliAiDataset(all_imgs)
        dl_test = DataLoader(ds_test, shuffle=False, batch_size=100, num_workers=4, pin_memory=True)

        for i,(imgs) in enumerate(dl_test):
            imgs = imgs.cuda()
            
            # fold 1
            grapheme_output_1,vowel_diacritic_output_1,consonant_diacritic_output_1 = model_1(imgs)
                        
            # fold2
            grapheme_output_2,vowel_diacritic_output_2,consonant_diacritic_output_2 = model_2(imgs)
            
            # fold 3
            grapheme_output_3,vowel_diacritic_output_3,consonant_diacritic_output_3 = model_3(imgs)
                       
            # fold 4
            grapheme_output_4,vowel_diacritic_output_4,consonant_diacritic_output_4 = model_4(imgs)
                        
            # fold5
            grapheme_output_5,vowel_diacritic_output_5,consonant_diacritic_output_5 = model_5(imgs)
                        
            # all folds
            grapheme_output = (grapheme_output_1+grapheme_output_2+grapheme_output_3+grapheme_output_4+grapheme_output_5)/5
            vowel_diacritic_output = (vowel_diacritic_output_1+vowel_diacritic_output_2+vowel_diacritic_output_3+vowel_diacritic_output_4+vowel_diacritic_output_5)/5
            consonant_diacritic_output = (consonant_diacritic_output_1+consonant_diacritic_output_2+consonant_diacritic_output_3+consonant_diacritic_output_4+consonant_diacritic_output_5)/5
            ...
            submission_grapheme_root_labels = submission_grapheme_root_labels.tolist()
            submission_vowel_diacritic_labels = submission_vowel_diacritic_labels.tolist()
            submission_consonant_diacritic_labels = submission_consonant_diacritic_labels.tolist()

            all_grapheme_labels = all_grapheme_labels+submission_grapheme_root_labels
            all_vowel_diacritic_labels = all_vowel_diacritic_labels+submission_vowel_diacritic_labels
            all_consonant_diacritic_labels = all_consonant_diacritic_labels+submission_consonant_diacritic_labels
...
row_id = []
target = []
for i in tqdm(range(len(all_vowel_diacritic_labels))):
    row_id += [f'Test_{i}_grapheme_root', f'Test_{i}_vowel_diacritic',
               f'Test_{i}_consonant_diacritic']
    target += [all_grapheme_labels[i], all_vowel_diacritic_labels[i], all_consonant_diacritic_labels[i]]
