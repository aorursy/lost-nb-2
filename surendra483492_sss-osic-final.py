...
num_epochs = 40
quantiles = (0.1, 0.5, 0.9) #(0.2, 0.5, 0.8)
model_name ='cauchy'
...
class ClinicalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cache_dir is None:
            ...
            patient_path = self.ctscans_dir / patient_id # SSS
            #print(patient_path / os.listdir(patient_path)[0]) SSS
            metadata = pydicom.read_file(patient_path / os.listdir(patient_path)[0])#pydicom.read_file(self.cache_dir / f'{patient_id}.dcm')
    ...
    def cache(self, cache_dir):
        ...
        for idx in bar:
            ...
            #torch.save(sample['metadata'], Path(cache_dir)/f'{patient_id}.dcm')
            
    def cache_train(self, cache_dir):
        Path(cache_dir).mkdir(exist_ok=True, parents=True)
        self.raw.to_csv(Path(cache_dir)/f'tabular_{self.mode}.csv', index=False)
...
#         ToTensor(),
#         ZeroCenter(pre_calculated_mean=0.029105728564346046),
#         GenerateLatentFeatures(autoencoder, latent_dir)
#     ]))
...
def group_split(dataset, groups, test_size=0.2):
    ...
    gss = GroupShuffleSplit(n_splits=5, test_size=test_size)
