...
get_ipython().run_cell_magic('capture', '', '!rm -r /opt/conda/lib/python3.6/site-packages/lightgbm\n!git clone --recursive https://github.com/Microsoft/LightGBM\n!apt-get install -y -qq libboost-all-dev')
...
get_ipython().run_cell_magic('capture', '', '!cd LightGBM/python-package/;python3 setup.py install --precompile\n!mkdir -p /etc/OpenCL/vendors && echo "libnvidia-opencl.so.1" > /etc/OpenCL/vendors/nvidia.icd\n!rm -r LightGBM')
...
def train_and_evaluate(): 
    ...
    data = pd.read_pickle('../input/timeseriessplit-cv-poisson/data_clean.pkl')
    data = transform(data)
    data['date'] = pd.to_datetime(data['date'])
...
get_ipython().run_cell_magic('time', '', "plt.rcParams['figure.facecolor'] = 'white'\ntrain_and_evaluate() ")
