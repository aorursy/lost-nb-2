...
train.groupby('PlayId')     .first()     .groupby('OffensePersonnel')     .count()['GameId']     .sort_values(ascending=False)     .head(15)     .sort_values()     .plot(kind='barh',
         title='Offense Personnel # of Plays',
         ax=ax[0])
train.groupby('PlayId')     .first()     .groupby('DefensePersonnel')     .count()['GameId']     .sort_values(ascending=False)     .head(15)     .sort_values()     .plot(kind='barh',
...
train.query("PlayId == @play_id and Team == 'away'")     .plot(x='X', y='Y', kind='scatter', ax=ax, color='orange', s=200, legend='Away')
train.query("PlayId == @play_id and Team == 'home'")     .plot(x='X', y='Y', kind='scatter', ax=ax, color='blue', s=200, legend='Home')
train.query("PlayId == @play_id and NflIdRusher == NflId")     .plot(x='X', y='Y', kind='scatter', ax=ax, color='red', s=200, legend='Rusher')
