...
LEARN_RATE = 0.0001 # [1e-4, 1e-3, 4e-3]
...
train_df = raw_train_df.groupby('class').    apply(lambda x: x.sample(TRAIN_SAMPLES//1)).    reset_index(drop=True)
...
print(accuracy_score(np.argmax(valid_Y, -1), np.argmax(pred_Y,-1)))
