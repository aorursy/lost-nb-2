...
for epoch in tqdm(range(epochs)):
    ...
    for ii, real_images in enumerate(train_loader):
        ...
        #errG += 0.01* torch.mean((featureF1 - torch.mean(featureR1,0))**2)
        errG += 0.01* torch.mean((featureF2 - torch.mean(featureR2,0))**2)
        errG.backward()
    ...
    print('[%d/%d]Loss_D: %.4f Loss_G: %.4f D(x): %.4f D(G(z)): %.4f / %.4f'
          % (epoch + 1, epochs, avgErrD/count, avgErrG/count, avgD_x/count, avgD_G_z1/count, avgD_G_z2/count))
    if((epoch+1)%5 == 0):
