...
# PATH = "../input/huggingfacetransformermodels/model_classes/roberta/roberta-base-tf2-model/"
...
if PATH == "../input/bert-base-uncased/":
    ...
    TOKENIZER = BertWordPieceTokenizer("../input/huggingfacetransformermodels/model_classes/roberta/roberta-base-tokenizer/vocab.json",lowercase=True, add_special_tokens=False)
...
class BertQAModel(TFBertPreTrainedModel):
    ...
    def call(self, inputs, **kwargs):
        ...
        tjl, _, hidden_states = self.bert(inputs, **kwargs)
        
#         hidden_states = self.concat([
#             hidden_states[-i] for i in range(1, self.NUM_HIDDEN_STATES+1)
#         ])
        
        hidden_states = self.dropout(tjl, training=kwargs.get("training", False))
        hidden_ouputs= self.hidden_output(hidden_states)
        hidden_outputs=self.dropout(hidden_ouputs,training=kwargs.get("training", False))
        logits = self.qa_outputs(hidden_outputs)
...
if PATH == "../input/bert-base-uncased/":
    ...
    config= BertConfig.from_json_file("../input/huggingfacetransformermodels/model_classes/roberta/roberta-base-tf2-model/config.json")
    print(type(config))
    config.output_hidden_states=True
BertQAModel.DROPOUT_RATE = 0.5
